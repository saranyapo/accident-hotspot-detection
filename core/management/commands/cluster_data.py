...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        query = "SELECT accident_id, city, location, latitude, longitude, custom_risk_score FROM accidents"
        ...
        for city in df['city'].unique():
            ...
            for cluster_id in range(chosen_k):
                ...
                # Use the dataset's own location name for this medoid point
                # (Kerala dataset gives place-level coordinates, e.g. "Vyttila
                # Junction" — no reverse geocoding needed)
                area_name = medoid_row['location'] if pd.notna(medoid_row['location']) else "Unknown Area"
